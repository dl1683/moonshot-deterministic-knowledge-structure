...
def _assert_duplicate_replay_invariance(
    replicas: list[KnowledgeStore],
    *,
    query_signature: Callable[[KnowledgeStore], tuple],
    assert_baseline_behavior: Callable[[KnowledgeStore, tuple], None],
) -> None:
    baseline_merged, baseline_conflicts = merge_replicas(replicas)
    baseline_signature = query_signature(baseline_merged)
    baseline_conflict_signatures = KnowledgeStore.conflict_signatures(baseline_conflicts)

    assert KnowledgeStore.conflict_code_counts(baseline_conflicts) == EXPECTED_CONFLICT_CODE_COUNTS
    assert_baseline_behavior(baseline_merged, baseline_signature)

    duplicate_merged, duplicate_conflicts = merge_replicas(
        replicas,
        start=baseline_merged,
    )
    duplicate_signature = query_signature(duplicate_merged)

    assert duplicate_signature == baseline_signature
    assert duplicate_conflicts == ()
    assert (
        KnowledgeStore.conflict_signatures(baseline_conflicts + duplicate_conflicts)
        == baseline_conflict_signatures
    )
    assert duplicate_merged.revision_state_signatures() == baseline_merged.revision_state_signatures()
    assert duplicate_merged.relation_state_signatures() == baseline_merged.relation_state_signatures()
    assert duplicate_merged.pending_relation_ids() == baseline_merged.pending_relation_ids()

    resumed_merged, resumed_conflicts = merge_replicas(
        replicas,
        start=baseline_merged.checkpoint(),
    )
    resumed_signature = query_signature(resumed_merged)

    assert resumed_signature == baseline_signature
    assert resumed_conflicts == ()
    assert (
        KnowledgeStore.conflict_signatures(baseline_conflicts + resumed_conflicts)
        == baseline_conflict_signatures
    )
    assert resumed_merged.revision_state_signatures() == baseline_merged.revision_state_signatures()
    assert resumed_merged.relation_state_signatures() == baseline_merged.relation_state_signatures()
    assert resumed_merged.pending_relation_ids() == baseline_merged.pending_relation_ids()


def _window_duplicate_replay_scenario() -> tuple[
    list[KnowledgeStore],
    datetime,
    int,
    int,
...
def test_query_revision_lifecycle_for_tx_window_duplicate_replay_idempotence_matches_single_shot() -> None:
    replicas, valid_at, tx_start, tx_end, _tx_from, _tx_to = _window_duplicate_replay_scenario()
    ...
    def assert_baseline_behavior(_store: KnowledgeStore, signature: tuple) -> None:
    ...
    _assert_duplicate_replay_invariance(
        replicas,
        query_signature=query_signature,
        assert_baseline_behavior=assert_baseline_behavior,
    )


def test_query_relation_resolution_for_tx_window_duplicate_replay_idempotence_matches_single_shot() -> None:
    replicas, valid_at, tx_start, tx_end, _tx_from, _tx_to = _window_duplicate_replay_scenario()
...
def test_query_relation_lifecycle_for_tx_window_duplicate_replay_idempotence_matches_single_shot() -> None:
    replicas, valid_at, tx_start, tx_end, _tx_from, _tx_to = _window_duplicate_replay_scenario()
...
def test_query_revision_lifecycle_transition_for_tx_window_duplicate_replay_idempotence_matches_single_shot() -> None:
    replicas, valid_at, _tx_start, _tx_end, tx_from, tx_to = _window_duplicate_replay_scenario()
    ...
    def assert_baseline_behavior(store: KnowledgeStore, signature: tuple) -> None:
...
def test_query_relation_resolution_transition_for_tx_window_duplicate_replay_idempotence_matches_single_shot() -> None:
    replicas, valid_at, _tx_start, _tx_end, tx_from, tx_to = _window_duplicate_replay_scenario()
...
def test_query_relation_lifecycle_transition_for_tx_window_duplicate_replay_idempotence_matches_single_shot() -> None:
    replicas, valid_at, _tx_start, _tx_end, tx_from, tx_to = _window_duplicate_replay_scenario()
