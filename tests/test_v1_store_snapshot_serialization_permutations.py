...
def _build_store_snapshot_serialization_replay_replicas(
    *,
    tx_base: int,
    ...
    core_subject = ClaimCore(
        ...
        slots={"subject": f"store-snapshot-serialization-subject-{tx_base}"},
    )
    core_anchor = ClaimCore(
        claim_type="document",
        slots={"id": f"store-snapshot-serialization-anchor-{tx_base}"},
    )
    core_context = ClaimCore(
        claim_type="fact",
        slots={"id": f"store-snapshot-serialization-context-{tx_base}"},
    )
    core_retracted = ClaimCore(
        claim_type="residence",
        slots={"subject": f"store-snapshot-serialization-retracted-{tx_base}"},
    )
    core_competing = ClaimCore(
        claim_type="residence",
        slots={"subject": f"store-snapshot-serialization-competing-{tx_base}"},
    )

    replica_base = KnowledgeStore()
    anchor_revision = replica_base.assert_revision(
        core=core_anchor,
        assertion="store snapshot serialization anchor",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 1, recorded_at=dt(2024, 1, 2)),
        provenance=Provenance(source="source_store_snapshot_serialization_anchor"),
        confidence_bp=9100,
        status="asserted",
    )
    context_revision = replica_base.assert_revision(
        core=core_context,
        assertion="store snapshot serialization context",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 1, recorded_at=dt(2024, 1, 2)),
        provenance=Provenance(source="source_store_snapshot_serialization_context"),
        confidence_bp=9100,
        status="asserted",
    )
    subject_revision = replica_base.assert_revision(
        core=core_subject,
        assertion="store snapshot serialization subject",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 2, recorded_at=dt(2024, 1, 3)),
        provenance=Provenance(source="source_store_snapshot_serialization_subject"),
        confidence_bp=8400,
        status="asserted",
    )
    replica_base.assert_revision(
        core=core_retracted,
        assertion="store snapshot serialization retracted asserted",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 2, recorded_at=dt(2024, 1, 3)),
        provenance=Provenance(
            source="source_store_snapshot_serialization_retracted_asserted"
        ),
        confidence_bp=8300,
        status="asserted",
    )
    replica_base.assert_revision(
        core=core_competing,
        assertion="store snapshot serialization competing a",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 4, recorded_at=dt(2024, 1, 5)),
        provenance=Provenance(source="source_store_snapshot_serialization_competing_a"),
        confidence_bp=8200,
        status="asserted",
    )
    ...
    replica_competing.assert_revision(
        core=core_competing,
        assertion="store snapshot serialization competing b",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 4, recorded_at=dt(2024, 1, 5)),
        provenance=Provenance(source="source_store_snapshot_serialization_competing_b"),
        confidence_bp=8200,
        status="asserted",
    )
    ...
    replica_updates.assert_revision(
        core=core_retracted,
        assertion="store snapshot serialization retracted final",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 6, recorded_at=dt(2024, 1, 7)),
        provenance=Provenance(source="source_store_snapshot_serialization_retracted_final"),
        confidence_bp=8300,
        status="retracted",
    )
    orphan_relation = RelationEdge(
        ...
        to_revision_id=f"missing-store-snapshot-serialization-endpoint-{tx_base}",
...
def _snapshot_canonicalization_signature(
    store: KnowledgeStore,
    *,
    tx_from: int,
    tx_to: int,
...
def test_store_snapshot_serialization_permutation_order_and_checkpoint_segmentation_are_equivalent() -> None:
    replicas, valid_at, tx_from, tx_to, subject_core_id = (
        _build_store_snapshot_serialization_replay_replicas(tx_base=7400)
    )
    ...
    for order in itertools.permutations(range(len(replicas))):
        ...
        unsplit_snapshot_signature = _snapshot_canonicalization_signature(
            unsplit_merged,
            tx_from=tx_from,
            tx_to=tx_to,
            valid_at=valid_at,
        ...
        if baseline_snapshot_signature is None:
            baseline_snapshot_signature = unsplit_snapshot_signature
        else:
            assert unsplit_snapshot_signature == baseline_snapshot_signature
        ...
        for boundaries in checkpoint_boundaries:
            ...
            segmented_snapshot_signature = _snapshot_canonicalization_signature(
                segmented_merged,
                tx_from=tx_from,
                tx_to=tx_to,
                valid_at=valid_at,
            ...
            assert segmented_snapshot_signature == unsplit_snapshot_signature


def test_store_snapshot_serialization_duplicate_replay_is_idempotent_for_equivalent_history() -> None:
    replicas, valid_at, tx_from, tx_to, subject_core_id = (
        _build_store_snapshot_serialization_replay_replicas(tx_base=7480)
    )
    query_core_ids: tuple[str | None, ...] = (None, subject_core_id)
    ...
    for order in itertools.permutations(range(len(replicas))):
        ...
        duplicate_snapshot_signature = _snapshot_canonicalization_signature(
            duplicate_merged,
            tx_from=tx_from,
            tx_to=tx_to,
            valid_at=valid_at,
        ...
        resumed_duplicate_snapshot_signature = _snapshot_canonicalization_signature(
        ...
        assert duplicate_snapshot_signature == unsplit_snapshot_signature
        assert resumed_duplicate_snapshot_signature == unsplit_snapshot_signature
