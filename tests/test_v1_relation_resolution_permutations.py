...
def _projection_signatures(
    projection: RelationResolutionProjection,
) -> tuple[
...
def _relation_resolution_checkpoint_scenario() -> tuple[
    list[KnowledgeStore],
    datetime,
    int,
    int,
    ...
    core_target = ClaimCore(claim_type="residence", slots={"subject": "resolution target"})
    core_doc_a = ClaimCore(claim_type="document", slots={"id": "resolution-doc-a"})
    core_doc_b = ClaimCore(claim_type="document", slots={"id": "resolution-doc-b"})
    core_competing = ClaimCore(claim_type="residence", slots={"subject": "resolution competing"})
    ...
    outside_window_pending = RelationEdge(
        relation_type="depends_on",
        from_revision_id=target_revision.revision_id,
        to_revision_id="missing-resolution-outside-window",
    ...
    window_pending = RelationEdge(
        relation_type="supports",
        from_revision_id=target_revision.revision_id,
        to_revision_id="missing-resolution-window",
        transaction_time=TransactionTime(tx_id=41, recorded_at=dt(2024, 1, 11)),
    )
...
def test_query_relation_resolution_as_of_checkpoint_resumed_permutation_replay_matches_unsplit() -> None:
    (
        replicas,
        valid_at,
        tx_as_of,
    ...
    ) = _relation_resolution_checkpoint_scenario()
    ...
    for order in itertools.permutations(range(len(replicas))):
        ...
        unsplit_projection = unsplit_merged.query_relation_resolution_as_of(
            tx_id=tx_as_of,
            valid_at=valid_at,
        )
        ...
        for split_index in range(1, len(ordered_replicas)):
            ...
            resumed_projection = resumed_merged.query_relation_resolution_as_of(
                tx_id=tx_as_of,
                valid_at=valid_at,
            )
...
def test_query_relation_resolution_for_tx_window_checkpoint_resumed_permutation_replay_matches_unsplit() -> None:
    (
        replicas,
        valid_at,
        _tx_as_of,
    ...
    for order in itertools.permutations(range(len(replicas))):
        ...
        unsplit_projection = unsplit_merged.query_relation_resolution_for_tx_window(
            tx_start=tx_start,
            tx_end=tx_end,
            valid_at=valid_at,
        )
        ...
        for split_index in range(1, len(ordered_replicas)):
            ...
            resumed_projection = resumed_merged.query_relation_resolution_for_tx_window(
                tx_start=tx_start,
                tx_end=tx_end,
                valid_at=valid_at,
            )
