...
SurfaceDeserializationRoundTripSignature = tuple[
    str,
    str,
    str,
    str,
...
def _build_state_fingerprint_deserialization_replay_replicas(
    *,
    tx_base: int,
    ...
    core_subject = ClaimCore(
        ...
        slots={"subject": f"state-fingerprint-deserialization-subject-{tx_base}"},
    )
    core_anchor = ClaimCore(
        claim_type="document",
        slots={"id": f"state-fingerprint-deserialization-anchor-{tx_base}"},
    )
    core_context = ClaimCore(
        claim_type="fact",
        slots={"id": f"state-fingerprint-deserialization-context-{tx_base}"},
    )
    core_retracted = ClaimCore(
        claim_type="residence",
        slots={"subject": f"state-fingerprint-deserialization-retracted-{tx_base}"},
    )
    core_competing = ClaimCore(
        claim_type="residence",
        slots={"subject": f"state-fingerprint-deserialization-competing-{tx_base}"},
    )

    replica_base = KnowledgeStore()
    anchor_revision = replica_base.assert_revision(
        core=core_anchor,
        assertion="state fingerprint deserialization anchor",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 1, recorded_at=dt(2024, 1, 2)),
        provenance=Provenance(source="source_state_fingerprint_deserialization_anchor"),
        confidence_bp=9100,
        status="asserted",
    )
    context_revision = replica_base.assert_revision(
        core=core_context,
        assertion="state fingerprint deserialization context",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 1, recorded_at=dt(2024, 1, 2)),
        provenance=Provenance(source="source_state_fingerprint_deserialization_context"),
        confidence_bp=9100,
        status="asserted",
    )
    subject_revision = replica_base.assert_revision(
        core=core_subject,
        assertion="state fingerprint deserialization subject",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 2, recorded_at=dt(2024, 1, 3)),
        provenance=Provenance(source="source_state_fingerprint_deserialization_subject"),
        confidence_bp=8400,
        status="asserted",
    )
    replica_base.assert_revision(
        core=core_retracted,
        assertion="state fingerprint deserialization retracted asserted",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 2, recorded_at=dt(2024, 1, 3)),
        provenance=Provenance(
            source="source_state_fingerprint_deserialization_retracted_asserted"
        ),
        confidence_bp=8300,
        status="asserted",
    )
    replica_base.assert_revision(
        core=core_competing,
        assertion="state fingerprint deserialization competing a",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 3, recorded_at=dt(2024, 1, 4)),
        provenance=Provenance(
            source="source_state_fingerprint_deserialization_competing_a"
        ),
        confidence_bp=8200,
        status="asserted",
    )
    ...
    replica_competing.assert_revision(
        core=core_competing,
        assertion="state fingerprint deserialization competing b",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 3, recorded_at=dt(2024, 1, 4)),
        provenance=Provenance(
            source="source_state_fingerprint_deserialization_competing_b"
        ),
        confidence_bp=8200,
        status="asserted",
    )
    ...
    replica_retracted.assert_revision(
        core=core_retracted,
        assertion="state fingerprint deserialization retracted final",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 6, recorded_at=dt(2024, 1, 7)),
        provenance=Provenance(
            source="source_state_fingerprint_deserialization_retracted_final"
        ),
        confidence_bp=8300,
        status="retracted",
    )
    ...
    orphan_relation = RelationEdge(
        ...
        from_revision_id=subject_revision.revision_id,
        to_revision_id=f"missing-state-fingerprint-deserialization-endpoint-{tx_base}",
...
def _surface_deserialization_round_trip_signature(
    ...
    *,
    ...
    valid_at: datetime,
    core_id: str | None,
) -> SurfaceDeserializationRoundTripSignature:
    ...
    as_of_round_trip = DeterministicStateFingerprint.from_canonical_json(
        as_of_canonical_json
    )
    window_round_trip = DeterministicStateFingerprint.from_canonical_json(
        window_canonical_json
    )
    transition_round_trip = DeterministicStateFingerprintTransition.from_canonical_json(
        transition_canonical_json
    )

    as_of_round_trip_canonical_json = as_of_round_trip.as_canonical_json()
    window_round_trip_canonical_json = window_round_trip.as_canonical_json()
    transition_round_trip_canonical_json = transition_round_trip.as_canonical_json()

    assert as_of_round_trip_canonical_json == as_of_canonical_json
    assert window_round_trip_canonical_json == window_canonical_json
    assert transition_round_trip_canonical_json == transition_canonical_json

    assert as_of_round_trip.digest == as_of_fingerprint.digest
    assert window_round_trip.digest == window_fingerprint.digest
    assert transition_round_trip.from_digest == transition_fingerprint.from_digest
    assert transition_round_trip.to_digest == transition_fingerprint.to_digest
    ...
    return (
        as_of_canonical_json,
        as_of_round_trip_canonical_json,
        as_of_fingerprint.digest,
        window_canonical_json,
        window_round_trip_canonical_json,
        window_fingerprint.digest,
        transition_canonical_json,
        transition_round_trip_canonical_json,
        transition_fingerprint.from_digest,
        transition_fingerprint.to_digest,
    )


def _assert_surface_deserialization_round_trip_equivalent(
    candidate: SurfaceDeserializationRoundTripSignature,
    expected: SurfaceDeserializationRoundTripSignature,
) -> None:
    (
        candidate_as_of_json,
        candidate_as_of_round_trip_json,
        candidate_as_of_digest,
        candidate_window_json,
        candidate_window_round_trip_json,
        candidate_window_digest,
        candidate_transition_json,
        candidate_transition_round_trip_json,
        candidate_transition_from_digest,
        candidate_transition_to_digest,
    ) = candidate
    (
        expected_as_of_json,
        expected_as_of_round_trip_json,
        expected_as_of_digest,
        expected_window_json,
        expected_window_round_trip_json,
        expected_window_digest,
        expected_transition_json,
        expected_transition_round_trip_json,
        expected_transition_from_digest,
        expected_transition_to_digest,
    ) = expected

    assert candidate_as_of_json == expected_as_of_json
    assert candidate_as_of_round_trip_json == expected_as_of_round_trip_json
    assert candidate_window_json == expected_window_json
    assert candidate_window_round_trip_json == expected_window_round_trip_json
    assert candidate_transition_json == expected_transition_json
    assert candidate_transition_round_trip_json == expected_transition_round_trip_json
...
def test_state_fingerprint_deserialization_permutation_order_and_checkpoint_segmentation_are_equivalent() -> None:
    replicas, valid_at, tx_from, tx_to, subject_core_id = (
        _build_state_fingerprint_deserialization_replay_replicas(tx_base=7000)
    )
    query_core_ids: tuple[str | None, ...] = (None, subject_core_id)
    baseline_surfaces_by_core: dict[str | None, SurfaceDeserializationRoundTripSignature] = (
        {}
    )
    ...
    checkpoint_boundaries = tuple(
        ...
        for boundaries in itertools.combinations(
            range(1, len(replicas)), checkpoint_count
        )
    )
    ...
    for order in itertools.permutations(range(len(replicas))):
        ...
        unsplit_surfaces_by_core: dict[
            str | None, SurfaceDeserializationRoundTripSignature
        ] = {}
        for query_core_id in query_core_ids:
            unsplit_surface = _surface_deserialization_round_trip_signature(
                unsplit_merged,
                tx_from=tx_from,
                tx_to=tx_to,
                valid_at=valid_at,
            ...
            if query_core_id not in baseline_surfaces_by_core:
                ...
                _assert_surface_deserialization_round_trip_equivalent(
                    unsplit_surface,
        ...
        for boundaries in checkpoint_boundaries:
            ...
            for query_core_id in query_core_ids:
                segmented_surface = _surface_deserialization_round_trip_signature(
                    segmented_merged,
                    tx_from=tx_from,
                    tx_to=tx_to,
                    valid_at=valid_at,
...
def test_state_fingerprint_deserialization_duplicate_replay_is_idempotent_for_equivalent_history() -> None:
    replicas, valid_at, tx_from, tx_to, subject_core_id = (
        _build_state_fingerprint_deserialization_replay_replicas(tx_base=7080)
    )
    query_core_ids: tuple[str | None, ...] = (None, subject_core_id)
    ...
    for order in itertools.permutations(range(len(replicas))):
        ...
        for query_core_id in query_core_ids:
            unsplit_surfaces_by_core[
                query_core_id
            ] = _surface_deserialization_round_trip_signature(
                unsplit_merged,
                tx_from=tx_from,
                tx_to=tx_to,
                valid_at=valid_at,
        ...
        for query_core_id in query_core_ids:
            duplicate_surface = _surface_deserialization_round_trip_signature(
                duplicate_merged,
                tx_from=tx_from,
                tx_to=tx_to,
                valid_at=valid_at,
            ...
            resumed_duplicate_surface = _surface_deserialization_round_trip_signature(
