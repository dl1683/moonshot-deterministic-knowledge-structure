...
def _build_state_fingerprint_replay_replicas(
    *,
    tx_base: int,
) -> tuple[list[KnowledgeStore], datetime, int, str]:
    ...
    core_subject = ClaimCore(
        ...
        slots={"subject": f"state-fingerprint-subject-{tx_base}"},
    )
    core_anchor = ClaimCore(
        claim_type="document",
        slots={"id": f"state-fingerprint-anchor-{tx_base}"},
    )
    core_context = ClaimCore(
        claim_type="fact",
        slots={"id": f"state-fingerprint-context-{tx_base}"},
    )
    core_retracted = ClaimCore(
        claim_type="residence",
        slots={"subject": f"state-fingerprint-retracted-{tx_base}"},
    )
    core_competing = ClaimCore(
        claim_type="residence",
        slots={"subject": f"state-fingerprint-competing-{tx_base}"},
    )

    replica_base = KnowledgeStore()
    anchor_revision = replica_base.assert_revision(
        core=core_anchor,
        assertion="state fingerprint anchor",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 1, recorded_at=dt(2024, 1, 2)),
        provenance=Provenance(source="source_state_fingerprint_anchor"),
        confidence_bp=9100,
        status="asserted",
    )
    context_revision = replica_base.assert_revision(
        core=core_context,
        assertion="state fingerprint context",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 1, recorded_at=dt(2024, 1, 2)),
        provenance=Provenance(source="source_state_fingerprint_context"),
        confidence_bp=9100,
        status="asserted",
    )
    subject_revision = replica_base.assert_revision(
        core=core_subject,
        assertion="state fingerprint subject",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 2, recorded_at=dt(2024, 1, 3)),
        provenance=Provenance(source="source_state_fingerprint_subject"),
        confidence_bp=8400,
        status="asserted",
    )
    replica_base.assert_revision(
        core=core_retracted,
        assertion="state fingerprint retracted asserted",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 2, recorded_at=dt(2024, 1, 3)),
        provenance=Provenance(source="source_state_fingerprint_retracted_asserted"),
        confidence_bp=8300,
        status="asserted",
    )
    replica_base.assert_revision(
        core=core_competing,
        assertion="state fingerprint competing a",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 3, recorded_at=dt(2024, 1, 4)),
        provenance=Provenance(source="source_state_fingerprint_competing_a"),
        confidence_bp=8200,
        status="asserted",
    )
    ...
    replica_competing.assert_revision(
        core=core_competing,
        assertion="state fingerprint competing b",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 3, recorded_at=dt(2024, 1, 4)),
        provenance=Provenance(source="source_state_fingerprint_competing_b"),
        confidence_bp=8200,
        status="asserted",
    )
    ...
    replica_retracted.assert_revision(
        core=core_retracted,
        assertion="state fingerprint retracted final",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 6, recorded_at=dt(2024, 1, 7)),
        provenance=Provenance(source="source_state_fingerprint_retracted_final"),
        confidence_bp=8300,
        status="retracted",
    )
    ...
    orphan_relation = RelationEdge(
        ...
        from_revision_id=subject_revision.revision_id,
        to_revision_id=f"missing-state-fingerprint-endpoint-{tx_base}",
...
def test_query_state_fingerprint_as_of_permutation_order_and_checkpoint_segmentation_are_equivalent() -> None:
    replicas, valid_at, tx_as_of, subject_core_id = _build_state_fingerprint_replay_replicas(
        tx_base=6200
    )
    ...
    for order in itertools.permutations(range(len(replicas))):
        ...
        for query_core_id in query_core_ids:
            unsplit_fingerprint = unsplit_merged.query_state_fingerprint_as_of(
                tx_id=tx_as_of,
                valid_at=valid_at,
        ...
        for boundaries in checkpoint_boundaries:
            ...
            for query_core_id in query_core_ids:
                segmented_fingerprint = segmented_merged.query_state_fingerprint_as_of(
                    tx_id=tx_as_of,
                    valid_at=valid_at,
                    core_id=query_core_id,
                )
...
def test_query_state_fingerprint_as_of_duplicate_replay_is_idempotent_for_equivalent_history() -> None:
    replicas, valid_at, tx_as_of, subject_core_id = _build_state_fingerprint_replay_replicas(
        tx_base=6280
    )
    query_core_ids: tuple[str | None, ...] = (None, subject_core_id)
    ...
    for order in itertools.permutations(range(len(replicas))):
        ...
        for query_core_id in query_core_ids:
            duplicate_fingerprint = duplicate_merged.query_state_fingerprint_as_of(
                tx_id=tx_as_of,
                valid_at=valid_at,
                core_id=query_core_id,
            )
            resumed_duplicate_fingerprint = (
                resumed_duplicate_merged.query_state_fingerprint_as_of(
                    tx_id=tx_as_of,
                    valid_at=valid_at,
                    core_id=query_core_id,
                )
