...
def _build_store_snapshot_file_io_replay_replicas(
    *,
    tx_base: int,
    ...
    core_subject = ClaimCore(
        ...
        slots={"subject": f"store-snapshot-file-io-subject-{tx_base}"},
    )
    core_anchor = ClaimCore(
        claim_type="document",
        slots={"id": f"store-snapshot-file-io-anchor-{tx_base}"},
    )
    core_context = ClaimCore(
        claim_type="fact",
        slots={"id": f"store-snapshot-file-io-context-{tx_base}"},
    )
    core_retracted = ClaimCore(
        claim_type="residence",
        slots={"subject": f"store-snapshot-file-io-retracted-{tx_base}"},
    )
    core_competing = ClaimCore(
        claim_type="residence",
        slots={"subject": f"store-snapshot-file-io-competing-{tx_base}"},
    )

    replica_base = KnowledgeStore()
    anchor_revision = replica_base.assert_revision(
        core=core_anchor,
        assertion="store snapshot file io anchor",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 1, recorded_at=dt(2024, 1, 2)),
        provenance=Provenance(source="source_store_snapshot_file_io_anchor"),
        confidence_bp=9100,
        status="asserted",
    )
    context_revision = replica_base.assert_revision(
        core=core_context,
        assertion="store snapshot file io context",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 1, recorded_at=dt(2024, 1, 2)),
        provenance=Provenance(source="source_store_snapshot_file_io_context"),
        confidence_bp=9100,
        status="asserted",
    )
    subject_revision = replica_base.assert_revision(
        core=core_subject,
        assertion="store snapshot file io subject",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 2, recorded_at=dt(2024, 1, 3)),
        provenance=Provenance(source="source_store_snapshot_file_io_subject"),
        confidence_bp=8400,
        status="asserted",
    )
    replica_base.assert_revision(
        core=core_retracted,
        assertion="store snapshot file io retracted asserted",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 2, recorded_at=dt(2024, 1, 3)),
        provenance=Provenance(source="source_store_snapshot_file_io_retracted_asserted"),
        confidence_bp=8300,
        status="asserted",
    )
    replica_base.assert_revision(
        core=core_competing,
        assertion="store snapshot file io competing a",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 4, recorded_at=dt(2024, 1, 5)),
        provenance=Provenance(source="source_store_snapshot_file_io_competing_a"),
        confidence_bp=8200,
        status="asserted",
    )
    ...
    replica_competing.assert_revision(
        core=core_competing,
        assertion="store snapshot file io competing b",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 4, recorded_at=dt(2024, 1, 5)),
        provenance=Provenance(source="source_store_snapshot_file_io_competing_b"),
        confidence_bp=8200,
        status="asserted",
    )
    ...
    replica_updates.assert_revision(
        core=core_retracted,
        assertion="store snapshot file io retracted final",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 6, recorded_at=dt(2024, 1, 7)),
        provenance=Provenance(source="source_store_snapshot_file_io_retracted_final"),
        confidence_bp=8300,
        status="retracted",
    )
    orphan_relation = RelationEdge(
        ...
        to_revision_id=f"missing-store-snapshot-file-io-endpoint-{tx_base}",
...
def _snapshot_file_io_signature(
    store: KnowledgeStore,
    *,
    snapshot_path: Path,
    tx_from: int,
    ...
    assert restored.as_canonical_payload() == canonical_payload
    ...
    restored_surface_signatures = tuple(
        _surface_snapshot_query_signature(
            restored,
            tx_from=tx_from,
            tx_to=tx_to,
    ...
    assert restored_surface_signatures == surface_signatures
...
def test_store_snapshot_file_io_permutation_order_and_checkpoint_segmentation_are_equivalent(
    tmp_path: Path,
) -> None:
    replicas, valid_at, tx_from, tx_to, subject_core_id = (
        _build_store_snapshot_file_io_replay_replicas(tx_base=7560)
    )
    ...
    for order in itertools.permutations(range(len(replicas))):
        ...
        unsplit_snapshot_signature = _snapshot_file_io_signature(
            unsplit_merged,
        ...
        for boundaries in checkpoint_boundaries:
            ...
            segmented_snapshot_signature = _snapshot_file_io_signature(
                segmented_merged,
                snapshot_path=snapshot_path,
                tx_from=tx_from,
                tx_to=tx_to,
...
def test_store_snapshot_file_io_duplicate_replay_is_idempotent_for_equivalent_history(
    tmp_path: Path,
) -> None:
    replicas, valid_at, tx_from, tx_to, subject_core_id = (
        _build_store_snapshot_file_io_replay_replicas(tx_base=7640)
    )
    snapshot_path = tmp_path / "snapshot.canonical.json"
    query_core_ids: tuple[str | None, ...] = (None, subject_core_id)
    ...
    for order in itertools.permutations(range(len(replicas))):
        ...
        duplicate_snapshot_signature = _snapshot_file_io_signature(
            duplicate_merged,
            snapshot_path=snapshot_path,
            tx_from=tx_from,
            tx_to=tx_to,
        ...
        resumed_duplicate_snapshot_signature = _snapshot_file_io_signature(
