...
def _build_store_snapshot_schema_version_replay_replicas(
    *,
    tx_base: int,
    ...
    core_subject = ClaimCore(
        ...
        slots={"subject": f"store-snapshot-schema-version-subject-{tx_base}"},
    )
    core_anchor = ClaimCore(
        claim_type="document",
        slots={"id": f"store-snapshot-schema-version-anchor-{tx_base}"},
    )
    core_context = ClaimCore(
        claim_type="fact",
        slots={"id": f"store-snapshot-schema-version-context-{tx_base}"},
    )
    core_retracted = ClaimCore(
        claim_type="residence",
        slots={"subject": f"store-snapshot-schema-version-retracted-{tx_base}"},
    )
    core_competing = ClaimCore(
        claim_type="residence",
        slots={"subject": f"store-snapshot-schema-version-competing-{tx_base}"},
    )

    replica_base = KnowledgeStore()
    anchor_revision = replica_base.assert_revision(
        core=core_anchor,
        assertion="store snapshot schema version anchor",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 1, recorded_at=dt(2024, 1, 2)),
        provenance=Provenance(source="source_store_snapshot_schema_version_anchor"),
        confidence_bp=9100,
        status="asserted",
    )
    context_revision = replica_base.assert_revision(
        core=core_context,
        assertion="store snapshot schema version context",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 1, recorded_at=dt(2024, 1, 2)),
        provenance=Provenance(source="source_store_snapshot_schema_version_context"),
        confidence_bp=9100,
        status="asserted",
    )
    subject_revision = replica_base.assert_revision(
        core=core_subject,
        assertion="store snapshot schema version subject",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 2, recorded_at=dt(2024, 1, 3)),
        provenance=Provenance(source="source_store_snapshot_schema_version_subject"),
        confidence_bp=8400,
        status="asserted",
    )
    replica_base.assert_revision(
        core=core_retracted,
        assertion="store snapshot schema version retracted asserted",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 2, recorded_at=dt(2024, 1, 3)),
        provenance=Provenance(
            source="source_store_snapshot_schema_version_retracted_asserted"
        ),
        confidence_bp=8300,
        status="asserted",
    )
    replica_base.assert_revision(
        core=core_competing,
        assertion="store snapshot schema version competing a",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 4, recorded_at=dt(2024, 1, 5)),
        provenance=Provenance(
            source="source_store_snapshot_schema_version_competing_a"
        ),
        confidence_bp=8200,
        status="asserted",
    )
    ...
    replica_competing.assert_revision(
        core=core_competing,
        assertion="store snapshot schema version competing b",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 4, recorded_at=dt(2024, 1, 5)),
        provenance=Provenance(
            source="source_store_snapshot_schema_version_competing_b"
        ),
        confidence_bp=8200,
        status="asserted",
    )
    ...
    replica_updates.assert_revision(
        core=core_retracted,
        assertion="store snapshot schema version retracted final",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 6, recorded_at=dt(2024, 1, 7)),
        provenance=Provenance(
            source="source_store_snapshot_schema_version_retracted_final"
        ),
        confidence_bp=8300,
        status="retracted",
    )
    orphan_relation = RelationEdge(
        ...
        to_revision_id=f"missing-store-snapshot-schema-version-endpoint-{tx_base}",
...
def _schema_versioned_snapshot_signature(
    store: KnowledgeStore,
    *,
    tx_from: int,
    tx_to: int,
    ...
    assert canonical_payload["snapshot_schema_version"] == 1
...
def test_store_snapshot_schema_version_permutation_checkpoint_and_duplicate_replay_are_invariant() -> None:
    replicas, valid_at, tx_from, tx_to, subject_core_id = (
        _build_store_snapshot_schema_version_replay_replicas(tx_base=7800)
    )
    ...
    for order in itertools.permutations(range(len(replicas))):
        ...
        unsplit_signature = _schema_versioned_snapshot_signature(
            unsplit_merged,
            tx_from=tx_from,
            tx_to=tx_to,
            valid_at=valid_at,
        ...
        for boundaries in checkpoint_boundaries:
            ...
            segmented_signature = _schema_versioned_snapshot_signature(
                segmented_merged,
                tx_from=tx_from,
                tx_to=tx_to,
                valid_at=valid_at,
        ...
        duplicate_signature = _schema_versioned_snapshot_signature(
            duplicate_merged,
            tx_from=tx_from,
            tx_to=tx_to,
            valid_at=valid_at,
        ...
        resumed_duplicate_signature = _schema_versioned_snapshot_signature(
...
def test_store_snapshot_schema_version_restart_save_load_is_invariant_for_equivalent_history(
    tmp_path: Path,
) -> None:
    replicas, valid_at, tx_from, tx_to, subject_core_id = (
        _build_store_snapshot_schema_version_replay_replicas(tx_base=7880)
    )
    query_core_ids: tuple[str | None, ...] = (None, subject_core_id)
    baseline_snapshot_signature = None
    baseline_conflict_signatures = None
    ...
    for order in itertools.permutations(range(len(replicas))):
        ...
        snapshot_path = tmp_path / f"schema-versioned-restart-{order_label}.canonical.json"
        for boundaries in restart_boundaries:
            ...
            restarted_signature = _schema_versioned_snapshot_signature(
                restarted_merged,
                tx_from=tx_from,
                tx_to=tx_to,
                valid_at=valid_at,
