...
_TARGET_PATH = _TESTS_DIR / "test_v1_store_snapshot_surface_parity_conflict_aware_replay_restart.py"
...
def _get_function(module: ast.Module, *, name: str) -> _DEF_OR_ASYNC:
    ...
    raise AssertionError(
        "Conflict-aware replay/restart runtime-guard target missing from tests/"
        "test_v1_store_snapshot_surface_parity_conflict_aware_replay_restart.py: "
        f"{name}"
    )
...
def test_conflict_aware_replay_restart_permutation_matrix_budget_guard() -> None:
    module = _load_module()
    function = _get_function(
        module,
        name="test_store_snapshot_surface_parity_conflict_aware_replay_restart_invariant_for_ingestion_permutations",
    )
    failures: list[str] = []
    ...
    if len(permutation_tuple_calls) != 1:
        failures.append(
            "Conflict-aware permutation matrix should build exactly one permutation tuple; "
    ...
    if len(permutation_len_asserts) != 1:
        failures.append(
            "Conflict-aware permutation matrix should enforce len(permutation_orders) == 24 "
    ...
    assert not failures, (
        "Conflict-aware replay/restart permutation runtime-budget guard drift detected in "
        "tests/test_v1_store_snapshot_surface_parity_conflict_aware_replay_restart.py: "
        + "; ".join(failures)
    )


def test_conflict_aware_replay_restart_checkpoint_duplicate_matrix_budget_guard() -> None:
    module = _load_module()
    function = _get_function(
        module,
        name="test_store_snapshot_surface_parity_conflict_aware_replay_restart_invariant_for_segmented_duplicate_and_restarts",
    )
    failures: list[str] = []
    ...
    assert not failures, (
        "Conflict-aware replay/restart checkpoint/duplicate runtime-budget guard drift detected in "
        "tests/test_v1_store_snapshot_surface_parity_conflict_aware_replay_restart.py: "
        + "; ".join(failures)
    )


def test_conflict_aware_replay_restart_cycle_budget_guard() -> None:
    module = _load_module()
    function = _get_function(
        module,
        name="test_store_snapshot_surface_parity_conflict_aware_replay_restart_invariant_for_segmented_duplicate_and_restarts",
    ...
    assert not failures, (
        "Conflict-aware replay/restart cycle runtime-budget guard drift detected in "
        "tests/test_v1_store_snapshot_surface_parity_conflict_aware_replay_restart.py: "
        + "; ".join(failures)
    )
