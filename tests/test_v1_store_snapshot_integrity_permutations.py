...
def _build_store_snapshot_integrity_replay_replicas(
    *,
    tx_base: int,
    ...
    core_subject = ClaimCore(
        ...
        slots={"subject": f"store-snapshot-integrity-subject-{tx_base}"},
    )
    core_anchor = ClaimCore(
        claim_type="document",
        slots={"id": f"store-snapshot-integrity-anchor-{tx_base}"},
    )
    core_context = ClaimCore(
        claim_type="fact",
        slots={"id": f"store-snapshot-integrity-context-{tx_base}"},
    )
    core_retracted = ClaimCore(
        claim_type="residence",
        slots={"subject": f"store-snapshot-integrity-retracted-{tx_base}"},
    )
    core_competing = ClaimCore(
        claim_type="residence",
        slots={"subject": f"store-snapshot-integrity-competing-{tx_base}"},
    )

    replica_base = KnowledgeStore()
    anchor_revision = replica_base.assert_revision(
        core=core_anchor,
        assertion="store snapshot integrity anchor",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 1, recorded_at=dt(2024, 1, 2)),
        provenance=Provenance(source="source_store_snapshot_integrity_anchor"),
        confidence_bp=9100,
        status="asserted",
    )
    context_revision = replica_base.assert_revision(
        core=core_context,
        assertion="store snapshot integrity context",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 1, recorded_at=dt(2024, 1, 2)),
        provenance=Provenance(source="source_store_snapshot_integrity_context"),
        confidence_bp=9100,
        status="asserted",
    )
    subject_revision = replica_base.assert_revision(
        core=core_subject,
        assertion="store snapshot integrity subject",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 2, recorded_at=dt(2024, 1, 3)),
        provenance=Provenance(source="source_store_snapshot_integrity_subject"),
        confidence_bp=8400,
        status="asserted",
    )
    replica_base.assert_revision(
        core=core_retracted,
        assertion="store snapshot integrity retracted asserted",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 2, recorded_at=dt(2024, 1, 3)),
        provenance=Provenance(source="source_store_snapshot_integrity_retracted_asserted"),
        confidence_bp=8300,
        status="asserted",
    )
    replica_base.assert_revision(
        core=core_competing,
        assertion="store snapshot integrity competing a",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 4, recorded_at=dt(2024, 1, 5)),
        provenance=Provenance(source="source_store_snapshot_integrity_competing_a"),
        confidence_bp=8200,
        status="asserted",
    )
    ...
    replica_competing.assert_revision(
        core=core_competing,
        assertion="store snapshot integrity competing b",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 4, recorded_at=dt(2024, 1, 5)),
        provenance=Provenance(source="source_store_snapshot_integrity_competing_b"),
        confidence_bp=8200,
        status="asserted",
    )
    ...
    replica_updates.assert_revision(
        core=core_retracted,
        assertion="store snapshot integrity retracted final",
        valid_time=valid_time,
        transaction_time=TransactionTime(tx_id=tx_base + 6, recorded_at=dt(2024, 1, 7)),
        provenance=Provenance(source="source_store_snapshot_integrity_retracted_final"),
        confidence_bp=8300,
        status="retracted",
    )
    orphan_relation = RelationEdge(
        ...
        to_revision_id=f"missing-store-snapshot-integrity-endpoint-{tx_base}",
...
def _snapshot_integrity_signature(
    store: KnowledgeStore,
    *,
    snapshot_path: Path,
    tx_from: int,
    ...
    assert snapshot_checksum == expected_snapshot_checksum
...
def test_store_snapshot_integrity_permutation_checkpoint_duplicate_and_restart_are_invariant(
    tmp_path: Path,
) -> None:
    replicas, valid_at, tx_from, tx_to, subject_core_id = (
        _build_store_snapshot_integrity_replay_replicas(tx_base=7960)
    )
    ...
    for order in itertools.permutations(range(len(replicas))):
        ...
        unsplit_signature = _snapshot_integrity_signature(
            unsplit_merged,
            snapshot_path=tmp_path / f"integrity-unsplit-{order_label}.canonical.json",
            tx_from=tx_from,
            tx_to=tx_to,
            valid_at=valid_at,
        ...
        for boundaries in checkpoint_boundaries:
            ...
            segmented_signature = _snapshot_integrity_signature(
                segmented_merged,
                snapshot_path=tmp_path / f"integrity-segmented-{order_label}.canonical.json",
                tx_from=tx_from,
                tx_to=tx_to,
                valid_at=valid_at,
                query_core_ids=query_core_ids,
        ...
        duplicate_signature = _snapshot_integrity_signature(
            duplicate_merged,
            snapshot_path=tmp_path / f"integrity-duplicate-{order_label}.canonical.json",
            tx_from=tx_from,
            tx_to=tx_to,
            valid_at=valid_at,
            query_core_ids=query_core_ids,
        )
        resumed_duplicate_signature = _snapshot_integrity_signature(
            resumed_duplicate_merged,
            snapshot_path=tmp_path
            / f"integrity-duplicate-checkpoint-{order_label}.canonical.json",
            tx_from=tx_from,
            tx_to=tx_to,
            valid_at=valid_at,
            query_core_ids=query_core_ids,
        ...
        for boundaries in restart_boundaries:
            restarted_merged, restarted_conflicts = replay_with_restart_snapshots(
                ordered_replicas,
                boundaries=boundaries,
                snapshot_path=tmp_path / f"integrity-restart-{order_label}.canonical.json",
            ...
            restarted_signature = _snapshot_integrity_signature(
                restarted_merged,
                snapshot_path=tmp_path / f"integrity-restarted-{order_label}.canonical.json",
                tx_from=tx_from,
                tx_to=tx_to,
                valid_at=valid_at,
                query_core_ids=query_core_ids,
