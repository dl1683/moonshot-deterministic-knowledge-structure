...
_TARGET_PATH = _TESTS_DIR / "test_v1_store_snapshot_merge_conflict_journal_replay_restart.py"
...
def _get_function(module: ast.Module, *, name: str) -> _DEF_OR_ASYNC:
    ...
    raise AssertionError(
        "Merge-conflict journal replay/restart runtime-guard target missing from tests/"
        "test_v1_store_snapshot_merge_conflict_journal_replay_restart.py: "
        f"{name}"
    )
...
def test_merge_conflict_journal_replay_restart_permutation_matrix_budget_guard() -> None:
    module = _load_module()
    function = _get_function(
        module,
        name="test_store_snapshot_merge_conflict_journal_replay_restart_invariant_for_ingestion_permutations",
    )
    failures: list[str] = []
    ...
    if len(permutation_tuple_calls) != 1:
        failures.append(
            "Merge-conflict journal permutation matrix should build exactly one permutation tuple; "
    ...
    if len(permutation_len_asserts) != 1:
        failures.append(
            "Merge-conflict journal permutation matrix should enforce len(permutation_orders) == 24 "
    ...
    replay_calls = [
        call
        for call in _iter_call_nodes(function)
        if _dotted_name(call.func) == "_replay_replicas_with_journal"
    ]
    if len(replay_calls) != 3:
        failures.append(
            "Permutation invariance test should keep exactly three _replay_replicas_with_journal call sites "
        ...
        for call in replay_calls:
            ...
            if literal != (1, 3):
                failures.append(
                    "_replay_replicas_with_journal segmented permutation variant should keep boundaries=(1, 3)"
                )
            if len(call.args) != 1 or _dotted_name(call.args[0]) != "ordered_replicas":
                failures.append(
                    "_replay_replicas_with_journal segmented permutation variant should keep ordered_replicas input routing"
                )

        if segmented_calls != 1:
            failures.append(
                "Permutation invariance test should keep exactly one segmented _replay_replicas_with_journal variant; "
                f"observed {segmented_calls}"
            )

    assert not failures, (
        "Merge-conflict journal replay/restart permutation runtime-budget guard drift detected in "
        "tests/test_v1_store_snapshot_merge_conflict_journal_replay_restart.py: "
        + "; ".join(failures)
    )


def test_merge_conflict_journal_replay_restart_checkpoint_duplicate_matrix_budget_guard() -> None:
    module = _load_module()
    function = _get_function(
        module,
        name="test_store_snapshot_merge_conflict_journal_replay_restart_invariant_for_segmented_duplicate_and_restarts",
    )
    failures: list[str] = []
    ...
    if len(replay_calls) != 2:
        failures.append(
            "Segmented/duplicate replay test should keep exactly two _replay_replicas_with_journal call sites "
        ...
        for call in replay_calls:
            ...
            if literal != (1, 3):
                failures.append(
                    "_replay_replicas_with_journal segmented replay variant should keep boundaries=(1, 3)"
                )
        if segmented_calls != 1:
            failures.append(
                "Segmented/duplicate replay test should keep exactly one segmented _replay_replicas_with_journal variant; "
                f"observed {segmented_calls}"
            )
    ...
    assert not failures, (
        "Merge-conflict journal replay/restart checkpoint/duplicate runtime-budget guard drift detected in "
        "tests/test_v1_store_snapshot_merge_conflict_journal_replay_restart.py: "
        + "; ".join(failures)
    )


def test_merge_conflict_journal_replay_restart_cycle_budget_guard() -> None:
    module = _load_module()
    function = _get_function(
        module,
        name="test_store_snapshot_merge_conflict_journal_replay_restart_invariant_for_segmented_duplicate_and_restarts",
    ...
    assert not failures, (
        "Merge-conflict journal replay/restart cycle runtime-budget guard drift detected in "
        "tests/test_v1_store_snapshot_merge_conflict_journal_replay_restart.py: "
        + "; ".join(failures)
    )
